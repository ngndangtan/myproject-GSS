...
extern 	uint8_t ST_MC;
extern 	uint8_t usart_receive_byte;
extern	unsigned flag_send_msg_finish;
extern 	uint8_t msg_index_count;
...
void TIM1_UP_IRQHandler(void)
{
	if(TIM_GetITStatus(TIM1, TIM_IT_Update) != RESET)
	{
		...
		if(ST_MC!=Check_message_from_QT)
		ST_MC=Send_message_usart;
	}
}
...
{
	uint8_t temp_uart_data;
	if(USART_GetITStatus(USART1,USART_IT_RXNE) != RESET)
	{
		temp_uart_data=USART_ReceiveData(USART1);
		usart_receive_byte=temp_uart_data;
		USART_ClearFlag(USART1, USART_FLAG_RXNE);
		ST_MC=Check_message_from_QT;
	}
}
